...
def hit_at_k(retrieved, relevant, k):
    """1.0 if a relevant chunk is in top-k"""
    return 1.0 if set(relevant) & set(retrieved[:k]) else 0.0


def recall_at_k(retrieved, relevant, k):
    """Proportion of all relevant chunks found in top-k"""
    if not relevant:
        return 0.0
...
def reciprocal_rank(retrieved, relevant):
    """1/rank of first relevant chunk (0 if not found)"""
...
def ndcg_at_k(retrieved, relevant, k):
    """DCG divided by ideal DCG — comparable across queries"""
...
def average(rows):
    """Average across queries (equal weight)"""
    if not rows:
        return {}
    return {
...
def print_table(results, metrics):
    """Display {run_name: scores} as a comparison table"""
    if not results:
        return
...
if __name__ == "__main__":
    # python -m evaluation.metrics — Test formulas with hand-calculated values
