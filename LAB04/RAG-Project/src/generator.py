...
class NoLLM:
    ...
    def chat(self, messages):
        ...
        # Extract block [1] content from prompt
...
class Generator:
    ...
    def generate(self, question, chunks, history=""):
        ...
        # Found nothing — say don't know, better than letting LLM guess
        if not chunks:
            return {
        ...
        try:
            ...
            #print(f"[llm] Call failed ({error}) — showing retrieved info instead")
    ...
    def build_sources(self, chunks):    # Create reference list, matching [1] [2] numbers in prompt
        sources = []
